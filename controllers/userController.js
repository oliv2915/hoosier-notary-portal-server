...
router.post("/register", (req, res) => {
    ...
    .then(userData => {
        return res.status(201).json({
            ...
            token: jwt.sign({id: userData.id}, process.env.JWT_SECRET, {expiresIn: 60 * 60 * 24})
        })
    })
    .catch(err => {
...
router.put("/update", validateToken, async (req, res) => {
    ...
    try {
        if (req.user.isEmployee && userId !== undefined) {
            ...
            if ((req.user.isEmployee && req.user.isSuper) && isSuper !== undefined) userDataToUpdate["isSuper"] = isSuper ? true : false;
            // update user flags
...
router.get("/profile", validateToken, (req, res) => {
    ...
    UserModel.findOne(query)
        ...
        .catch(err => res.status(500).json({message: "Server error getting user profile"}))
});
