...
router.post("/add", validateToken, (req, res) => {
    if (!req.user.isEmployee) return res.status(401).json({message: "Not Authorized"});
    const {fileNumber, dueDate, notes, contactName, contactPhoneNumber, contactEmail, meetingAddress, rate, type, status, customerId} = req.body.assignment;
    ...
    if (!contactPhoneNumber) missingRequiredFields.push("contactPhoneNumber is required");
    ...
    .catch(err => res.status(500).json({message: "Error adding assignment"}))
});
/*
    Update Assignment
...
router.put("/update", validateToken, (req, res) => {
    const {fileNumber, dueDate, notes, contactName, contactPhoneNumber, contactEmail, meetingAddress, rate, type, status, customerId, notaryId, assignmentId} = req.body.assignment;
    // check for missing required fields
    const missingRequiredFields = [];
    if (!customerId) missingRequiredFields.push("customerId is required");
    ...
    if (req.user.isEmployee) fieldsToUpdate = {
        fileNumber, dueDate, contactName,
        contactPhoneNumber, contactEmail, meetingAddress,
        rate, type, status,
    ...
    .catch(err => res.status(500).json({message: "Error updating assignment"}))
});
/*
...
router.get("/", validateToken, (req, res) => {
    ...
    .catch(err => res.status(500).json({message: "Error getting assignment"}))
