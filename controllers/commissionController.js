...
router.post("/add", validateToken, (req, res) => {
	...
	if (!commissionNumber)
		missingRequiredFields.push("commissionNumber is required");
	if (!nameOnCommission)
		missingRequiredFields.push("nameOnCommission is required");
	if (!commissionExpireDate)
		missingRequiredFields.push("commissionExpireDate is required");
	if (!commissionState)
		missingRequiredFields.push("commissionState is required");
	if (!countyOfResidence)
		missingRequiredFields.push("countyOfresidence is required");
	...
	CommissionModel.create({
		...
		.then((commission) =>
			res.status(201).json({ message: "Commission created successfully" })
		)
		.catch((err) => {
