...
router.post("/add", validateToken, (req, res) => {
    ...
    ContactModel.create({
      ...
      .then(customer => res.status(201).json({message: "Contact created successfully"}))
      .catch(err => {
          if (err instanceof UniqueConstraintError) {
              return res.status(409).json({message: "UniqueConstraintError", error: "Email address already in use"});
          } else if (err instanceof ValidationError) {
              return res.status(400).json({message: "ValidationError", error: err.errors});
          } else {
...
router.put("/update", validateToken, (req, res) => {
    ...
    .then(result => res.status(200).json({message: "Contact updated successfully"}))
    .catch(err => res.status(500).json({message: "Error updating contact"}))
...
router.delete("/delete", validateToken, (req, res) => {
    ...
    .then(result => {
        if (result === 0) return res.status(200).json({message: "No contact found to delete"});
        if (result === 1) return res.status(200).json({message: "Contact removed successfully"});
    })
    .catch(err => res.status(500).json({message: "Error deleting contact"}))
});
