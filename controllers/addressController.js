...
router.post("/add", validateToken, (req, res) => {
    const {streetOne, streetTwo, city, state, zipCode, type, customerId} = req.body.address;
    ...
    if (req.user.isEmployee && customerId === undefined) missingRequiredFields.push("customerId is required");
...
router.put("/update", validateToken, async (req, res) => {
    const {streetOne, streetTwo, city, state, zipCode, type, addressId, customerId} = req.body.address;
    // check for missing required fields
    const missingRequiredFields = [];
    if (!addressId) missingRequiredFields.push("addressId is required");
    if (req.user.isEmployee && !customerId) missingRequiredFields.push("customerId is required");
    if (missingRequiredFields.length > 0) return res.status(400).json({message: missingRequiredFields});
    ...
    try {
        ...
        const foundAddress = await AddressModel.findOne(query).then(address => address.get());
        // only update if values have changed
        const updateResult = await AddressModel.update({
            streetOne: (streetOne === foundAddress.streetOne) ? foundAddress.streetOne : streetOne,
            streetTwo: (streetTwo === foundAddress.streetTwo) ? foundAddress.streetTwo : (streetTwo !== undefined) ? streetTwo : null,
            city: (city === foundAddress.city) ? foundAddress.city : city,
            state: (state === foundAddress.state) ? foundAddress.state : state,
            zipCode: (zipCode === foundAddress.zipCode) ? foundAddress.zipCode : zipCode,
            type: (type === foundAddress.type) ? foundAddress.type : type
        }, query);
        // if update successful, return a 200 status
        if (updateResult > 0) return res.status(200).json({message: "Address updated successfully"});
    } catch (err) {
        if (err instanceof ValidationError) {
            return res.status(400).json({
                message: "ValidationError",
...
router.delete("/delete", validateToken, (req, res) => {
    ...
    .then(result => {
        if (result === 0) return res.status(200).json({message: "No address found to delete"});
        if (result === 1) return res.status(200).json({message: "Address removed successfully"});
    })
